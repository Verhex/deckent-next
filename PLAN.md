...
| Worker / sandbox | [Docker](src/adapters/core/docker-supervisor), [execution composition](src/composition/core/execution) | Gerçek görev yolu Git+Docker; ağ kapalı, kısıtlı mount/yetki/kaynak. Process supervisor modülü var; tmux/uzak/Windows/Firebase ürün kanıtı yok. Ortak kernel izolasyonu VM garantisi değildir. |
| Workspace / kaynak sürümü | [Git workspace](src/adapters/core/git-workspace), [workspace motoru](src/engine/core/workspaces) | Attempt başına bağımsız Git clone, detached base, origin kaldırma, hook kapatma; `git worktree` implementasyonu değil. Run base sabitliği mevcut; canlı hedefe güvenli birleştirme/ERP teslimi eksik. |
