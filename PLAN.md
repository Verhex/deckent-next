...
PROVIDER-SPEND persistence/inspection foundation (preparation only): strict monetary records, SQLite20 atomic reservation custody and inspection6 are staged without registering operator spending config or enforcing budgets in the production invocation application. Mandatory enforcement remains incomplete until trusted native quote production, config registration, the application gate and composition land together.

PROVIDER-NATIVE/SPENDING — REVIEW candidate: config-scoped budget + native HTTPS tariff acquisition + shared credential transport + mandatory application admission; SDK/CLI/MCP/runtime regression evidence. Historical replay remains source-independent. Native usage settlement, real-provider metadata completeness and paid acceptance remain open. No new dogfood/remote/platform claim.
...
| PROVIDER-SPEND/DURABLE-AUDIT | ACCOUNT-SNAPSHOT; ledger21 | Explicit separately authorized bounded consistency audit, immutable actor/policy/checkpoint receipt, ledger22 migration, runtime10 SDK/CLI/MCP delivery, inspection2 current/stale projection | REVIEW |

Audit limits come from optional versioned provider_spend_audit config; a child cannot expand parent ceilings. No defaults grant audit authority. This slice does not schedule audits, correct settled charges or verify vendor invoices. Current/stale refers to checkpoint identity, not general invocation history. Independent review is required before completion.
