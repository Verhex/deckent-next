...
it('refuses opening a schema2 reader, then reads only after the writer migrates to current schema', async () => {
  ...
  try { expect(migrated.prepare('PRAGMA user_version').get()!.user_version).toBe(13); } finally { migrated.close(); }
});
...
it('requires migration before reading schema-eleven Run snapshots and leaves inspection bytes unchanged', async () => {
  ...
  downgradeRunEligibilityFixtures(db); db.exec('DROP TABLE model_activation_receipts; DROP TABLE model_activations; PRAGMA user_version=11'); db.close();
