...
it('refuses opening a schema2 reader, then reads only after the writer migrates to current schema', async () => {
  const path = await fixture(); const db = new DatabaseSync(path); db.exec('DROP TABLE service_shutdown_commands; DROP TABLE service_shutdown_outcomes; DROP TABLE cancellation_deliveries; DROP TABLE run_workspace_custody; DROP TABLE runs; DROP TABLE run_receipts; DROP TABLE execution_pools; PRAGMA user_version=2'); db.close();
  ...
  try { expect(migrated.prepare('PRAGMA user_version').get()!.user_version).toBe(10); } finally { migrated.close(); }
});
