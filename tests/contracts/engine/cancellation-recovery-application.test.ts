...
import { CancellationRecoveryApplication, PolicyAuthorizationError } from '#engine/index.js';
...
function harness(page: readonly ReturnType<typeof identity>[], options: { scopeDeny?: boolean; runDeny?: (id: string) => boolean; attemptDeny?: (id: string) => boolean; concurrency?: number } = {}) {
  ...
  const store = {
    ...
    async loadCancellationDispatch(value: ReturnType<typeof identity>) { return { request: { identity: value } } as never; },
  ...
  const run = { async authorize(_action: unknown, query: { runId: string }) { calls.runs.push(query.runId); if (options.runDeny?.(query.runId)) throw new PolicyAuthorizationError('POLICY_DENIED'); } };
  const dispatch = {
...
it('denies a Run before claiming delivery or consuming its retry budget', async () => {
  ...
  const result = await f.app.drain({ schemaVersion: 1, scopeId: 's', afterAttemptId: null });
  expect(result.outcomes[0]!.outcome.status).toBe('denied'); expect(f.calls.claims).toBe(0); expect(f.calls.cancels).toEqual([]); expect(f.calls.cancelCommands).toBe(0);
});
