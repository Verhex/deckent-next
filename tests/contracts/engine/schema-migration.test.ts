...
it('advances a populated current Run ledger after its version is marked four', async () => {
  ...
  try {
    ...
    store.close();
    const downgrade = new DatabaseSync(path); downgrade.exec('DROP TABLE cancellation_deliveries; DROP TABLE run_workspace_custody; PRAGMA user_version=4'); downgrade.close();
...
it('requires an installed synchronous profile validator before migrating dispatch custody to six', async () => {
  ...
  try {
    ...
    const downgrade = new DatabaseSync(path); downgrade.exec('DROP TABLE cancellation_deliveries; DROP TABLE run_workspace_custody; PRAGMA user_version=5'); downgrade.close();
    await expect(openSqliteAttemptStore(path, options)).rejects.toMatchObject({ code: 'LEDGER_RESET_REQUIRED' });
    ...
    const after = new DatabaseSync(path, { readOnly: true }); try { expect(after.prepare('PRAGMA user_version').get()!.user_version).toBe(9); } finally { after.close(); }
  } finally { await rm(root, { recursive: true, force: true }); }
});
