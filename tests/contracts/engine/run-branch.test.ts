...
it('upgrades ledger22 without fabricating decisions for ordinary Runs or rewriting receipts', async () => {
  ...
  const db = new DatabaseSync(f.path); db.exec('DROP TABLE IF EXISTS run_execution_intents; DROP TABLE IF EXISTS task_evaluation_observations; DROP TABLE IF EXISTS workspace_integrations; DROP TABLE IF EXISTS approval_outbox; DROP TABLE IF EXISTS approval_receipts; DROP TABLE IF EXISTS approvals; PRAGMA user_version=22'); db.close();
