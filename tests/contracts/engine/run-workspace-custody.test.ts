...
it('migrates schema eight by adding an empty custody table without inventing records', async () => {
  ...
  try { expect(check.prepare('PRAGMA user_version').get()!.user_version).toBe(11); expect(check.prepare('SELECT count(*) AS count FROM run_workspace_custody').get()!.count).toBe(0); }
  finally { check.close(); }
});
