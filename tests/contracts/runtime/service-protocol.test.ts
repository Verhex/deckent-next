...
import { RuntimeServiceProtocolError, classifyRuntimeServiceOperation, isRuntimeServiceStreamingOperation, parseRuntimeServiceResponse, runtimeServiceOperationSchema,
  runtimeServiceRequestSchema, runtimeServiceResponseSchema, runtimeServiceResultCapacity, runtimeServiceStreamFrameSchema } from '../../../src/engine/core/runtime/index.js';
...
describe('runtime service protocol', () => {
  it('accepts exactly the current v11 operation allowlist', () => {
  ...
  it('streams an invocation with bounded delivery and strict ordered delta frames', () => {
    const stream = { schemaVersion: 11, requestId: 'request-1', operation: 'invokeModelStream', input: invocation, delivery: { maxResultBytes: 1 } };
    ...
    const frame = { schemaVersion: 11, requestId: 'request-1', kind: 'delta', sequence: 0, deltas: [{ kind: 'reasoning', text: 'a' }, { kind: 'text', text: 'b' }] };
    ...
    for (const invalid of [{ ...frame, deltas: [] }, { ...frame, deltas: [{ kind: 'text', text: '' }] }, { ...frame, deltas: [{ kind: 'tool', text: 'x' }] },
      { ...frame, sequence: -1 }, { ...frame, extra: 1 }, { ...frame, schemaVersion: 10 },
  ...
  it('requires input to be present and rejects old or extra request fields', () => {
    const request = { schemaVersion: 11, requestId: 'request-1', operation: 'inspectRun', input: null };
    ...
    expect(() => runtimeServiceRequestSchema.parse({ ...request, schemaVersion: 10 })).toThrow();
    expect(() => runtimeServiceRequestSchema.parse({ ...request, input: undefined })).not.toThrow();
    const withoutInput = { schemaVersion: 11, requestId: 'request-1', operation: 'inspectRun' };
  ...
  it('requires bounded delivery only for model invocation operations and validates descriptor-safe inputs', () => {
    const invoke = { schemaVersion: 11, requestId: 'request-1', operation: 'invokeModel', input: invocation,
      delivery: { maxResultBytes: 1 } };
    expect(runtimeServiceRequestSchema.parse(invoke)).toEqual(invoke);
    expect(runtimeServiceRequestSchema.parse({ schemaVersion: 11, requestId: 'request-2', operation: 'inspectModelInvocation',
    ...
    expect(() => runtimeServiceRequestSchema.parse({ schemaVersion: 11, requestId: 'request-3', operation: 'inspectRun',
  ...
  it('derives an exact v11 success-envelope result capacity and rejects impossible or invalid limits', () => {
    ...
    const exact = { schemaVersion: 11, requestId, ok: true, result: 'x'.repeat(capacity - 2) };
    const overflow = { schemaVersion: 11, requestId, ok: true, result: 'x'.repeat(capacity - 1) };
  ...
  it('requires success results and keeps failures closed and message-free', () => {
    const success = { schemaVersion: 11, requestId: 'request-1', ok: true, result: null };
    expect(runtimeServiceResponseSchema.parse(success)).toEqual(success);
    const withoutResult = { schemaVersion: 11, requestId: 'request-1', ok: true };
    expect(() => runtimeServiceResponseSchema.parse(withoutResult)).toThrow();
    const failure = { schemaVersion: 11, requestId: 'request-1', ok: false,
  ...
  it('validates response correlation after validating the response envelope', () => {
    const response = { schemaVersion: 11, requestId: 'request-1', ok: true, result: {} };
