...
describe('runtime service protocol', () => {
  it('accepts exactly the current v12 operation allowlist', () => {
  ...
  it('streams an invocation with bounded delivery and strict ordered delta frames', () => {
    const stream = { schemaVersion: 12, requestId: 'request-1', operation: 'invokeModelStream', input: invocation, delivery: { maxResultBytes: 1 } };
    ...
    const frame = { schemaVersion: 12, requestId: 'request-1', kind: 'delta', sequence: 0, deltas: [{ kind: 'reasoning', text: 'a' }, { kind: 'text', text: 'b' }] };
    ...
    for (const invalid of [{ ...frame, deltas: [] }, { ...frame, deltas: [{ kind: 'text', text: '' }] }, { ...frame, deltas: [{ kind: 'tool', text: 'x' }] },
      { ...frame, sequence: -1 }, { ...frame, extra: 1 }, { ...frame, schemaVersion: 11 },
  ...
  it('admits a chat turn only with bounded delivery and a history that ends with the user, and carries its events in strict event frames', () => {
    const turn = { schemaVersion: 12, requestId: 'request-1', operation: 'chatTurn', delivery: { maxResultBytes: 4096 },
    ...
    const cancel = { schemaVersion: 12, requestId: 'request-2', operation: 'cancelChatTurn', delivery: { maxResultBytes: 256 },
    ...
    const frame = { schemaVersion: 12, requestId: 'request-1', kind: 'event', sequence: 0, events: [
    ...
    for (const invalid of [{ ...frame, events: [{ kind: 'done', finish: 'stop', note: null }] }, { ...frame, events: [] }, { ...frame, kind: 'delta' },
      { ...frame, events: [{ kind: 'tool.finished', callId: 'c1', name: 'read_file', status: 'maybe', ms: 1, bytes: 1 }] }, { ...frame, schemaVersion: 11 }]) {
  ...
  it('requires input to be present and rejects old or extra request fields', () => {
    const request = { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: null };
    ...
    expect(() => runtimeServiceRequestSchema.parse({ ...request, schemaVersion: 11 })).toThrow();
    expect(() => runtimeServiceRequestSchema.parse({ ...request, input: undefined })).not.toThrow();
    const withoutInput = { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun' };
  ...
  it('requires bounded delivery only for model invocation operations and validates descriptor-safe inputs', () => {
    const invoke = { schemaVersion: 12, requestId: 'request-1', operation: 'invokeModel', input: invocation,
      delivery: { maxResultBytes: 1 } };
    expect(runtimeServiceRequestSchema.parse(invoke)).toEqual(invoke);
    expect(runtimeServiceRequestSchema.parse({ schemaVersion: 12, requestId: 'request-2', operation: 'inspectModelInvocation',
    ...
    expect(() => runtimeServiceRequestSchema.parse({ schemaVersion: 12, requestId: 'request-3', operation: 'inspectRun',
  ...
  it('derives an exact v11 success-envelope result capacity and rejects impossible or invalid limits', () => {
    ...
    const exact = { schemaVersion: 12, requestId, ok: true, result: 'x'.repeat(capacity - 2) };
    const overflow = { schemaVersion: 12, requestId, ok: true, result: 'x'.repeat(capacity - 1) };
  ...
  it('requires success results and keeps failures closed and message-free', () => {
    const success = { schemaVersion: 12, requestId: 'request-1', ok: true, result: null };
    expect(runtimeServiceResponseSchema.parse(success)).toEqual(success);
    const withoutResult = { schemaVersion: 12, requestId: 'request-1', ok: true };
    expect(() => runtimeServiceResponseSchema.parse(withoutResult)).toThrow();
    const failure = { schemaVersion: 12, requestId: 'request-1', ok: false,
  ...
  it('validates response correlation after validating the response envelope', () => {
    const response = { schemaVersion: 12, requestId: 'request-1', ok: true, result: {} };
