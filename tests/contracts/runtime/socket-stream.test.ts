...
const streamRequest = (requestId = 'request-1') => ({ schemaVersion: 11 as const, requestId, operation: 'invokeModelStream' as const,
...
describe.skipIf(process.platform !== 'linux')('local runtime socket streaming', () => {
  it('delivers ordered coalesced delta frames before exactly one final response, and only to streamed operations', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async (request, _peer, stream) => {
      ...
      return { schemaVersion: 11, requestId: request.requestId, ok: true, result: { done: true } };
    });
    try {
      ...
      expect(response).toEqual({ schemaVersion: 11, requestId: 'request-1', ok: true, result: { done: true } });
      ...
      await expect(requestLocalRuntime(options, { schemaVersion: 11, requestId: 'request-2', operation: 'inspectRun', input: {} }))
        .resolves.toMatchObject({ ok: true });
      expect(channels[0]).toBeDefined(); expect(channels[1]).toBeUndefined();
      await expect(streamLocalRuntime(options, { schemaVersion: 11, requestId: 'request-3', operation: 'inspectRun', input: {} }, () => undefined))
  ...
  it('rejects out-of-order, foreign, missing-final and trailing frames from a peer', async () => {
    ...
    const delta = (sequence: number, requestId = 'request-1') => frame({ schemaVersion: 11, requestId, kind: 'delta', sequence, deltas: [{ kind: 'text', text: 'x' }] });
    const final = frame({ schemaVersion: 11, requestId: 'request-1', ok: true, result: null });
