...
describe.skipIf(process.platform !== 'linux')('local runtime socket request admission', () => {
  it('rejects an oversized complete request before it connects to the owned endpoint', async () => {
    ...
    try {
      await expect(requestLocalRuntime({ ...options, inputMaxBytes: 64 }, { schemaVersion: 8, requestId: 'request-1',
  ...
  it('rejects a pre-aborted client signal without dispatching a handler', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => {
      calls++; return { schemaVersion: 8, requestId: request.requestId, ok: true, result: null };
    });
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }, signal.signal))
  ...
  it('serves one correlated framed request per connection with strict endpoint permissions', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 8,
    ...
    try {
      ...
      await expect(requestLocalRuntime(options, { schemaVersion: 8, requestId: 'request-1',
  ...
  it('does not dispatch malformed or extra frames', async () => {
    ...
    let calls = 0;
    const server = await startLocalRuntimeSocketServer(options, async request => {
      calls += 1; return { schemaVersion: 8, requestId: request.requestId, ok: true, result: null };
    });
    try {
      const socket = await connect(options.endpoint);
      const frame = encodeServiceFrame({ schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }, 4096);
...
describe.skipIf(process.platform !== 'linux')('local runtime socket peer identity and endpoint custody', () => {
  it('delivers immutable kernel peer identity instead of client supplied actor fields', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async (request, peer) => {
      ...
      return { schemaVersion: 8, requestId: request.requestId, ok: true, result: peer };
    });
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 8, requestId: 'peer-proof',
  ...
  it('releases the guard when endpoint cleanup is refused without deleting the replacement', async () => {
    const { options } = await fixture();
    const handler = async (request: { requestId: string }) => ({ schemaVersion: 8 as const,
  ...
  it('holds exclusive ownership and releases it on ordered close', async () => {
    const { options } = await fixture();
    const first = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 8,
      requestId: request.requestId, ok: true, result: null }));
    try {
      await expect(startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 8,
    ...
    const restarted = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 8,
      requestId: request.requestId, ok: true, result: null }));
  ...
  it('never deletes an unsafe endpoint file or symlink', async () => {
    ...
    await expect(startLocalRuntimeSocketServer(regular.options, async request => ({ schemaVersion: 8,
      requestId: request.requestId, ok: true, result: null }))).rejects.toSatisfy(error => {
    ...
    await expect(startLocalRuntimeSocketServer(linked.options, async request => ({ schemaVersion: 8,
      requestId: request.requestId, ok: true, result: null }))).rejects.toSatisfy(error => {
        expectCode(error, 'LOCAL_RUNTIME_ENDPOINT_UNSAFE'); return true;
      });
...
describe.skipIf(process.platform !== 'linux')('local runtime socket response delivery', () => {
  it('disconnects only the aborting client after admission while its held handler settles once', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => {
      ...
      return { schemaVersion: 8, requestId: request.requestId, ok: true, result: null };
    });
    try {
      const pending = requestLocalRuntime(options, { schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }, controller.signal);
  ...
  it('continues an admitted handler after the client disconnects', async () => {
    ...
    try {
      const socket = await connect(options.endpoint);
      socket.end(encodeServiceFrame({ schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }, 4096));
      socket.destroy();
      await completed;
    } finally { await server.dispose(); }
  });
  ...
  it('hands a wrapped reply off once after its successful response flush', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => ({ response: { schemaVersion: 8,
      requestId: request.requestId, ok: true, result: null }, afterResponseOrDisconnect() { handoffs++; } }));
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }))
  ...
  it('hands a wrapped reply off after a client disconnects before its handler resolves', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => {
      entered(); await gate;
      return { response: { schemaVersion: 8, requestId: request.requestId, ok: true, result: null }, afterResponseOrDisconnect() { handoffs++; } };
    });
    try {
      const socket = await connect(options.endpoint);
      socket.end(encodeServiceFrame({ schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }, 4096));
  ...
  it('delivers a typed response limit and hands the admitted effect off exactly once', async () => {
    ...
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }))
        .resolves.toEqual({ schemaVersion: 8, requestId: 'request-1', ok: false, error: { code: 'RUNTIME_SERVICE_RESPONSE_LIMIT', category: 'error' } });
      for (let i = 0; i < 20 && handoffs === 0; i++) await new Promise(resolve => setTimeout(resolve, 1));
      expect(handoffs).toBe(1);
    } finally { await server.dispose(); }
  });
  ...
  it('checks the actual correlated error envelope before dispatch at the exact byte boundary', async () => {
    ...
    const expected = { schemaVersion: 8, requestId, ok: false, error: { code: 'RUNTIME_SERVICE_RESPONSE_LIMIT', category: 'error' } };
    ...
    for (const responseMaxBytes of [errorBytes - 1, errorBytes]) {
      ...
      const server = await startLocalRuntimeSocketServer(bounded, async request => {
        effects++;
        return { schemaVersion: 8, requestId: request.requestId, ok: true, result: 'x'.repeat(options.responseMaxBytes) };
      });
      try {
        const response = requestLocalRuntime(bounded, { schemaVersion: 8, requestId, operation: 'inspectRun', input: {} });
  ...
  it('rejects a handler response with the wrong correlation identity', async () => {
    const { options } = await fixture();
    const server = await startLocalRuntimeSocketServer(options, async () => ({ schemaVersion: 8,
      requestId: 'foreign-request', ok: true, result: 'private' }));
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 8, requestId: 'request-1', operation: 'inspectRun', input: {} }))
        .resolves.toEqual({ schemaVersion: 8, requestId: 'request-1', ok: false,
