...
describe.skipIf(process.platform !== 'linux')('local runtime socket request admission', () => {
  it('rejects an oversized complete request before it connects to the owned endpoint', async () => {
    ...
    try {
      await expect(requestLocalRuntime({ ...options, inputMaxBytes: 64 }, { schemaVersion: 12, requestId: 'request-1',
  ...
  it('rejects a pre-aborted client signal without dispatching a handler', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => {
      calls++; return { schemaVersion: 12, requestId: request.requestId, ok: true, result: null };
    });
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }, signal.signal))
  ...
  it('serves one correlated framed request per connection with strict endpoint permissions', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 12,
    ...
    try {
      ...
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'request-1',
  ...
  it('keeps describe and shutdown reachable in the previous protocol version and answers in that version; nothing else', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => {
      seen.push(`${request.schemaVersion}:${request.operation}`); return { schemaVersion: 12, requestId: request.requestId, ok: true, result: { operation: request.operation } };
    });
    try {
      // An older (v10) client describing a service started from a newer build gets a v10 answer.
      await expect(requestLocalRuntime(options, { schemaVersion: 11, requestId: 'request-1', operation: 'describeService', input: {} }))
      ...
      await expect(requestLocalRuntime(options, { schemaVersion: 11, requestId: 'request-2', operation: 'inspectRun', input: {} } as never)).rejects.toThrow();
      const raw = await connect(options.endpoint);
      raw.end(encodeServiceFrame({ schemaVersion: 11, requestId: 'request-3', operation: 'inspectRun', input: {} }, options.inputMaxBytes));
      await once(raw, 'close');
      expect(seen).toEqual(['12:describeService']);
    } finally { await server.dispose(); }
  });

  it('carries bounded typed-error parameters in the current and the previous (v11) lifecycle envelope', async () => {
    const { options } = await fixture();
    const server = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 12, requestId: request.requestId, ok: false,
      error: { code: 'CONFIG_VALIDATION', category: 'config', params: { issues: 'terminal.scopeId: unrecognized key' } } }));
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }))
        .resolves.toMatchObject({ ok: false, error: { code: 'CONFIG_VALIDATION', params: { issues: 'terminal.scopeId: unrecognized key' } } });
      const older = await requestLocalRuntime(options, { schemaVersion: 11, requestId: 'request-2', operation: 'describeService', input: {} });
      // The client normalizes the previous version's envelope to the current one; the parameters survive the round trip.
      expect(older).toEqual({ schemaVersion: 12, requestId: 'request-2', ok: false,
        error: { code: 'CONFIG_VALIDATION', category: 'config', params: { issues: 'terminal.scopeId: unrecognized key' } } });
    } finally { await server.dispose(); }
  });
  ...
  it('does not dispatch malformed or extra frames', async () => {
    ...
    let calls = 0;
    const server = await startLocalRuntimeSocketServer(options, async request => {
      calls += 1; return { schemaVersion: 12, requestId: request.requestId, ok: true, result: null };
    });
    try {
      const socket = await connect(options.endpoint);
      const frame = encodeServiceFrame({ schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }, 4096);
...
describe.skipIf(process.platform !== 'linux')('local runtime socket peer identity and endpoint custody', () => {
  it('delivers immutable kernel peer identity instead of client supplied actor fields', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async (request, peer) => {
      ...
      return { schemaVersion: 12, requestId: request.requestId, ok: true, result: peer };
    });
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'peer-proof',
  ...
  it('authenticates a live native peer and revokes its session when that connection closes', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async (request, peer) => {
      ...
      return { schemaVersion: 12, requestId: request.requestId, ok: true, result: { active: true } };
    });
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'session-proof',
  ...
  it('rejects a fully disconnected live client before a privileged mutation while allowing request half-close', async () => {
    ...
    try {
      client.end(encodeServiceFrame({ schemaVersion: 12, requestId: 'disconnect', operation: 'inspectRun', input: {} }, options.inputMaxBytes));
      await admitted;
  ...
  it('releases the guard when endpoint cleanup is refused without deleting the replacement', async () => {
    const { options } = await fixture();
    const handler = async (request: { requestId: string }) => ({ schemaVersion: 12 as const,
  ...
  it('holds exclusive ownership and releases it on ordered close', async () => {
    const { options } = await fixture();
    const first = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 12,
      requestId: request.requestId, ok: true, result: null }));
    try {
      await expect(startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 12,
    ...
    const restarted = await startLocalRuntimeSocketServer(options, async request => ({ schemaVersion: 12,
      requestId: request.requestId, ok: true, result: null }));
  ...
  it('never deletes an unsafe endpoint file or symlink', async () => {
    ...
    await expect(startLocalRuntimeSocketServer(regular.options, async request => ({ schemaVersion: 12,
      requestId: request.requestId, ok: true, result: null }))).rejects.toSatisfy(error => {
    ...
    await expect(startLocalRuntimeSocketServer(linked.options, async request => ({ schemaVersion: 12,
      requestId: request.requestId, ok: true, result: null }))).rejects.toSatisfy(error => {
        expectCode(error, 'LOCAL_RUNTIME_ENDPOINT_UNSAFE'); return true;
      });
...
describe.skipIf(process.platform !== 'linux')('local runtime socket response delivery', () => {
  it('disconnects only the aborting client after admission while its held handler settles once', async () => {
    ...
    try {
      const pending = requestLocalRuntime(options, { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }, controller.signal);
  ...
  it('continues an admitted handler after the client disconnects', async () => {
    ...
    try {
      const socket = await connect(options.endpoint);
      socket.end(encodeServiceFrame({ schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }, 4096));
      socket.destroy();
      await completed;
    } finally { await server.dispose(); }
  });
  ...
  it('hands a wrapped reply off once after its successful response flush', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => ({ response: { schemaVersion: 12,
  ...
  it('hands a wrapped reply off after a client disconnects before its handler resolves', async () => {
    ...
    const server = await startLocalRuntimeSocketServer(options, async request => {
      entered(); await gate;
      return { response: { schemaVersion: 12, requestId: request.requestId, ok: true, result: null }, afterResponseOrDisconnect() { handoffs++; } };
    });
    try {
      const socket = await connect(options.endpoint);
      socket.end(encodeServiceFrame({ schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }, 4096));
  ...
  it('delivers a typed response limit and hands the admitted effect off exactly once', async () => {
    ...
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }))
        .resolves.toEqual({ schemaVersion: 12, requestId: 'request-1', ok: false, error: { code: 'RUNTIME_SERVICE_RESPONSE_LIMIT', category: 'error' } });
      for (let i = 0; i < 20 && handoffs === 0; i++) await new Promise(resolve => setTimeout(resolve, 1));
      expect(handoffs).toBe(1);
    } finally { await server.dispose(); }
  });
  ...
  it('checks the actual correlated error envelope before dispatch at the exact byte boundary', async () => {
    ...
    const expected = { schemaVersion: 12, requestId, ok: false, error: { code: 'RUNTIME_SERVICE_RESPONSE_LIMIT', category: 'error' } };
    ...
    for (const responseMaxBytes of [errorBytes - 1, errorBytes]) {
      ...
      const server = await startLocalRuntimeSocketServer(bounded, async request => {
        effects++;
        return { schemaVersion: 12, requestId: request.requestId, ok: true, result: 'x'.repeat(options.responseMaxBytes) };
      });
      try {
        const response = requestLocalRuntime(bounded, { schemaVersion: 12, requestId, operation: 'inspectRun', input: {} });
  ...
  it('rejects a handler response with the wrong correlation identity', async () => {
    const { options } = await fixture();
    const server = await startLocalRuntimeSocketServer(options, async () => ({ schemaVersion: 12,
      requestId: 'foreign-request', ok: true, result: 'private' }));
    try {
      await expect(requestLocalRuntime(options, { schemaVersion: 12, requestId: 'request-1', operation: 'inspectRun', input: {} }))
        .resolves.toEqual({ schemaVersion: 12, requestId: 'request-1', ok: false,
