...
describe.skipIf(process.platform === 'win32')('configured SDK cancellation intent', () => {
  ...
  it('never upgrades an older ledger during cancellation', async () => {
    const f = await fixture(); const db = new DatabaseSync(f.path); db.exec('DROP TABLE model_invocation_spend_reservations; DROP TABLE provider_spend_accounts; DROP TABLE execution_pools; PRAGMA user_version=3'); db.close();
