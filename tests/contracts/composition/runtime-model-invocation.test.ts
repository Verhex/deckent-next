...
it.skipIf(process.platform !== 'linux')('owns bounded invocation through current wire9 across disconnect, replay, restart and policy change', async () => {
  ...
  raw.end(encodeServiceFrame({ schemaVersion: 9, requestId: randomUUID(), operation: 'invokeModel', input: held,
  ...
  const forgedResponse = await requestLocalRuntime({ endpoint: service.endpoint, ...f.serviceOptions }, {
    schemaVersion: 9, requestId: randomUUID(), operation: 'invokeModel', input: forged,
  ...
  expirySocket.end(encodeServiceFrame({ schemaVersion: 9, requestId: randomUUID(), operation: 'invokeModel', input: expiring,
    delivery: { maxResultBytes: 60_000 } }, 65536));
