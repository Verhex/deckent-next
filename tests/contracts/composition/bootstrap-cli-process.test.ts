...
import { hashBootstrapJournal, productResourcePath, resolveProductLayout } from '#platform/index.js';
...
it('compiled CLI rejects incomplete installation before autoheal and keeps global inspection independent', async () => {
  ...
  try {
    ...
    const payload = { schemaVersion: 1 as const, transactionId: 'cli-partial-install', planDigest: 'a'.repeat(64), profileDigest: 'b'.repeat(64),
      phase: 'pending' as const, createdAtMs: 0, updatedAtMs: 0, resources: [{ resource: 'config', path: config,
        preimageDigest: null, targetDigest: 'c'.repeat(64), state: 'pending' as const }], blockers: ['IMAGE_PROVENANCE_UNVERIFIED'] };
    const bytes = JSON.stringify({ ...payload, checksum: hashBootstrapJournal(payload) });
