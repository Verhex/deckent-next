...
describe.skipIf(process.platform === 'win32')('configured SDK Run admission', () => {
  ...
  it('refuses schema upgrade during SDK admission and leaves the older ledger unchanged', async () => {
    ...
    db.exec('DROP TABLE model_invocation_spend_reservations; DROP TABLE provider_spend_accounts; DROP TABLE execution_pools; DROP TABLE IF EXISTS workspace_integrations; PRAGMA user_version=3'); db.close(); const before = await readFile(f.path);
