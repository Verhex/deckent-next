...
import type { ModelInvocationCommand, ModelInvocationPurgeCommand, ModelInvocationQuery } from '#domain/index.js';
import type { ModelInvocationInspection, ModelInvocationPurgeResult, ModelInvocationResult } from '#engine/index.js';
...
async function fixture(include = true) {
  const calls: { invoke: unknown[]; inspect: unknown[]; purge: unknown[] } = { invoke: [], inspect: [], purge: [] };
...
it('forwards only parsed command and query objects to injected applications', async () => {
  ...
  expect(f.calls).toEqual({ invoke: [command], inspect: [query], purge: [purge] });
});
...
it('rejects malformed or extended invocation input before either application is called', async () => {
  ...
  expect(f.calls).toEqual({ invoke: [], inspect: [], purge: [] });
});
