...
it.each(['en', 'tr'])('makes partial cancellation visible in %s without changing JSON or command success', async language => {
  const outcomes = states.map((status, index) => ({ taskId: 't' + index, attemptId: 'a' + index, status }));
  const response = { schemaVersion: 1 as const, layout: resolveProductPaths('/fixture/project', { env: { HOME: '/fixture/home' } }),
    delivery: { schemaVersion: 1 as const, runId: 'r', scopeId: 's', cancellationRequested: true as const, outcomes } };
  ...
  expect(text).toContain(language === 'en' ? 'termination not confirmed for 3 of 5 attempts' : '5 girişimin 3 tanesinde sonlanma doğrulanmadı');
  ...
  expect(text).toContain(language === 'en' ? 'No recorded attempts' : 'kayıtlı girişim yok');
});
