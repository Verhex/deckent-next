...
async function seed(path: string, version = 11) {
  ...
  db.exec(`DROP TABLE IF EXISTS run_execution_intents; DROP TABLE IF EXISTS task_evaluation_observations; PRAGMA user_version=${version}`); db.close(); return expected;
}
