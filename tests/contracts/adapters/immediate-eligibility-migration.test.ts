...
async function seed(path: string, version = 11) {
  ...
  db.exec(`DROP TABLE IF EXISTS run_execution_intents; DROP TABLE IF EXISTS task_evaluation_observations; DROP TABLE IF EXISTS workspace_integrations; PRAGMA user_version=${version}`); db.close(); return expected;
}
