...
it('rejects an old ledger without changing its bytes', async () => {
  const path = await ledger(), db = new DatabaseSync(path); db.exec('DROP TABLE IF EXISTS workspace_integrations; DROP TABLE IF EXISTS approval_outbox; DROP TABLE IF EXISTS approval_receipts; DROP TABLE IF EXISTS approvals; PRAGMA user_version=20'); db.close();
  const before = await readFile(path);
...
it('rejects a genuine schema-21 ledger without an audit table or changing bytes', async () => {
  ...
  try { db.exec('DROP TABLE provider_spend_audits; DROP TABLE IF EXISTS workspace_integrations; DROP TABLE IF EXISTS approval_outbox; DROP TABLE IF EXISTS approval_receipts; DROP TABLE IF EXISTS approvals; PRAGMA user_version=21'); } finally { db.close(); }
  const before = await readFile(path);
  await expect(openSqliteProviderSpendAccountReader(path, readerOptions)).rejects.toMatchObject({ code: 'ATTEMPT_STORE_VERSION' });
  await expect(readFile(path)).resolves.toEqual(before);
});
