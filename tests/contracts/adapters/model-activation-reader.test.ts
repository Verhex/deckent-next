...
it('accepts a genuine historical v13 activation ledger read-only without changing its bytes', async () => {
  ...
  db.exec(`DROP TABLE provider_spend_audits; DROP TABLE model_invocation_spend_reservations; DROP TABLE provider_spend_accounts;
    ...
    DROP TABLE model_invocation_contents; DROP TABLE model_invocation_content_purges; DROP TABLE model_invocations; DROP TABLE model_invocation_allocations; DROP TABLE IF EXISTS workspace_integrations; DROP TABLE IF EXISTS approval_outbox; DROP TABLE IF EXISTS approval_receipts; DROP TABLE IF EXISTS approvals; PRAGMA user_version=13`);
  db.close();
...
it('does not create a missing database and rejects an older schema without changing it', async () => {
  ...
  const old = await path(), db = new DatabaseSync(old); db.exec('DROP TABLE IF EXISTS workspace_integrations; DROP TABLE IF EXISTS approval_outbox; DROP TABLE IF EXISTS approval_receipts; DROP TABLE IF EXISTS approvals; PRAGMA user_version=12'); db.close(); const before = await readFile(old);
