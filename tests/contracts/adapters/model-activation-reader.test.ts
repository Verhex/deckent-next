...
it('does not create a missing database and rejects an older schema without changing it', async () => {
  ...
  const old = await path(), db = new DatabaseSync(old); db.exec('DROP TABLE IF EXISTS workspace_integrations; DROP TABLE IF EXISTS workspace_deliveries; DROP TABLE IF EXISTS workspace_adoptions; DROP TABLE IF EXISTS effect_intents; DROP TABLE IF EXISTS approval_outbox; DROP TABLE IF EXISTS approval_receipts; DROP TABLE IF EXISTS approvals; PRAGMA user_version=12'); db.close(); const before = await readFile(old);
