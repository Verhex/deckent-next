...
describe('model invocation domain contract', () => {
  ...
  it('retains only request evidence and rejects profile protocols outside the exact definition', () => {
    ...
    const receipt = { schemaVersion: 2, request, actor: { id: 'actor', issuer: 'os', subject: '1', assurance: 'os-user' },
    ...
    expect(() => parseModelInvocationReceipt({ ...receipt, schemaVersion: 1 })).toThrow('MODEL_INVOCATION_INVALID');
