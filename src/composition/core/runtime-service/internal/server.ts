  ...
  const server = await startLocalRuntimeSocketServer(socketOptions(config.service, endpoint), async (request, peer) => {
    try {
      if (request.operation === 'describeService') {
        ...
        return { schemaVersion: 4, requestId: request.requestId, ok: true, result };
      }
      if (request.operation === 'shutdownService') {
        ...
        return { response: { schemaVersion: 4, requestId: request.requestId, ok: true, result },
      ...
      return { schemaVersion: 4, requestId: request.requestId, ok: false, error: { code: failure.code, category: failure.category } };
    }
  });
