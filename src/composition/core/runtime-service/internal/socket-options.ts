...
export function socketOptions(settings: Omit<LocalRuntimeSocketOptions, 'endpoint'>, endpoint: string): LocalRuntimeSocketOptions {
  return { endpoint, maxConnections: settings.maxConnections, inputMaxBytes: settings.inputMaxBytes,
    responseMaxBytes: settings.responseMaxBytes, headerTimeoutMs: settings.headerTimeoutMs, responseTimeoutMs: settings.responseTimeoutMs,
    acceptRetryDelayMs: settings.acceptRetryDelayMs, acceptRetryLimit: settings.acceptRetryLimit };
}
