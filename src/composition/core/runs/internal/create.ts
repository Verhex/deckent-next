...
import { capExecutionSlots, resolveExecutionRegistry, RunAdmissionApplication, runAdmissionSchema, RunPolicyAuthorization, PoolPolicyAuthorization, type RunAdmission, type RunCreate } from '#engine/index.js';
import { loadConfiguredInferenceProfile } from '#composition/core/inference-serving/index.js';
  ...
  try {
    ...
    const app = new RunAdmissionApplication(store, { async verify() { return principal; } },
      new RunPolicyAuthorization({ async load() { return document; } }), new PoolPolicyAuthorization({ async load() { return document; } }), { async resolve(admitted) {
        ...
        const executionSlots = inferenceProfile ? capExecutionSlots(inferenceProfile, profile.executionSlots) : profile.executionSlots;
        ...
        return { execution, layoutRevision: layout.revision, now: Date.now(), policy: { schemaVersion: 2, poolId: profile.poolId,
          capacity: { executionSlots, inFlightSlots: profile.inFlightSlots }, ordering: admitted.graph.tasks.map(task => task.id) } };
      } });
