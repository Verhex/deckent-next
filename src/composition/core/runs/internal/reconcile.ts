...
import { authenticate, DispatchApplication, DispatchPolicyAuthorization, DispatchError } from '#engine/index.js';
...
export async function reconcileConfiguredAttempt(projectRoot: string, input: AttemptIdentity, options: ConfigLoadOptions = {}) {
  try {
    ...
    try {
      const recorded = await store.loadBoundDispatch(identity);
