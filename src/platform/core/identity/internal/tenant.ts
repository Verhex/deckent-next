...
import { resolveProductPaths, productResourcePath, type PathContext } from '#platform/core/host/index.js';
import { pathApi } from '#platform/core/host/index.js';
import { envValue } from '#platform/core/host/index.js';
...
export function tenantIsolationPath(projectRoot: string, tenantId: string, context: PathContext = {}): string {
  ...
  return pathApi(context.platform ?? process.platform).join(productResourcePath(resolveProductPaths(projectRoot, context), 'tenants'), tenantId);
}
