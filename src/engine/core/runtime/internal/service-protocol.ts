...
// v3 local transport is same-OS-UID only. Requests never provide an actor; current peer policy supplies scope.
// Invocation results carry an advisory replay flag; it is not independent evidence of spend or permission to retry.
export const runtimeServiceRequestSchema = z.object({ schemaVersion: z.literal(3), requestId: identitySchema,
...
const success = z.object({ schemaVersion: z.literal(3), requestId: identitySchema, ok: z.literal(true), result: z.unknown() })
  .strict().refine(value => Object.hasOwn(value, 'result'), { path: ['result'], message: 'RUNTIME_SERVICE_RESULT_REQUIRED' }).readonly();
const failure = z.object({ schemaVersion: z.literal(3), requestId: identitySchema, ok: z.literal(false),
...
/** Maximum serialized result bytes that still fit the v3 success envelope and an optional caller cap. */
export function runtimeServiceResultCapacity(requestId: string, responseMaxBytes: number, requestedMaxResultBytes?: number): number {
  ...
  const envelopeBytes = BigInt(Buffer.byteLength(JSON.stringify({ schemaVersion: 3, requestId: id, ok: true, result: null }), 'utf8'));
