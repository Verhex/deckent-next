...
import { createImmutableJsonObjectSchema, MODEL_INVOCATION_NATIVE_JSON_LIMITS,
...
// Headroom applies only to the descriptor-safe outer copy; verifyModelInvocationReceipt
// independently reapplies the canonical receipt limits.
const envelope = createImmutableJsonObjectSchema({
  maxDepth: MODEL_INVOCATION_NATIVE_JSON_LIMITS.maxDepth + 1,
  maxNodes: MODEL_INVOCATION_NATIVE_JSON_LIMITS.maxNodes * 2,
  maxCodeUnits: MODEL_INVOCATION_NATIVE_JSON_LIMITS.maxCodeUnits * 2,
});
...
const inspectionSchema = z.object({ schemaVersion: z.literal(1), scopeId: z.string(), invocationId: z.string(),
  reference: z.unknown(), invocation: z.unknown().nullable() }).strict();
...
export function parseModelInvocationResultForCommand(commandInput: unknown, input: unknown): ModelInvocationResult {
  try {
    ...
    const receipt = verifyModelInvocationReceipt(parsed.data.receipt);
...
export function parseModelInvocationInspectionForQuery(queryInput: unknown, input: unknown): ModelInvocationInspection {
  try {
    ...
    if (parsed.data.invocation === null) return Object.freeze({ ...query, invocation: null });
    const receipt: ModelInvocationReceipt = verifyModelInvocationReceipt(parsed.data.invocation);
    ...
    return Object.freeze({ ...query, invocation: receipt });
  } catch (error) {
    if (error instanceof ModelInvocationStoreError) throw error;
    return corrupt();
  }
