import { parseModelInvocationQuery, type ModelInvocationQuery, type ModelInvocationReceipt, type ModelInvocationResponseContent } from '#domain/index.js';
...
export type ModelInvocationInspection = Readonly<{ schemaVersion: 2; scopeId: string; invocationId: string;
  reference: ModelInvocationQuery['reference']; invocation: ModelInvocationReceipt | null;
  contentStatus: 'retained' | 'not-captured' | null; responseContent?: ModelInvocationResponseContent | null }>;
export class ModelInvocationInspectionApplication {
  ...
  async inspect(input: unknown, credential?: unknown, delivery?: ModelInvocationDelivery): Promise<ModelInvocationInspection> {
    ...
    try {
      ...
      if (!stored) return checkInvocationResultDelivery(Object.freeze({ ...identity, invocation: null, contentStatus: null,
      ...
      return checkInvocationResultDelivery(Object.freeze({ ...identity, invocation: receipt,
        contentStatus: record.content === null ? 'not-captured' : 'retained',
