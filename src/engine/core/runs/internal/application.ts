...
export class RunApplication {
  constructor(private readonly store: Pick<RunStore, 'loadRun' | 'cancelRun'>, private readonly verifier: PrincipalVerifier,
    private readonly authorization: RunAuthorization) {}
  async inspect(input: unknown, credential?: unknown) {
    ...
    return this.store.loadRun(query.scopeId, query.runId);
  }
