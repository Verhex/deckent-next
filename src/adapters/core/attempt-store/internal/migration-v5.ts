...
import { dispatchRecordSchema, runCreateSchema, AttemptStoreError } from '#engine/index.js';
...
export function requireLedgerV5Custody(db: DatabaseSync): void {
  try {
    ...
    for (const row of db.prepare('SELECT scope_id,snapshot,command FROM run_receipts').iterate()) {
      ...
      requireRunReceiptCommand(row.command);
    }
...
function requireRunSnapshot(value: unknown) {
  return runSnapshotSchema.parse(JSON.parse(String(value)));
}

function requireRunReceiptCommand(value: unknown): void {
