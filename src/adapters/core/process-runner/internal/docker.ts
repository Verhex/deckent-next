...
import { DockerCommandFailure, type DockerCommandRunner } from '#adapters/core/docker-supervisor/index.js';
...
export function createScopedNodeDockerRunner(profile: Pick<ProcessCommand, 'cwd' | 'env'>): DockerCommandRunner {
  ...
  return async (command, signal) => {
    const result = await runNodeProcess({ schemaVersion: 1, requestId: randomUUID(), ...command, cwd, env }, signal);
