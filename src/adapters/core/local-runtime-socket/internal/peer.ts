...
export type LocalPeerIdentity = Readonly<{ pid: number; uid: number; gid: number; assurance: 'linux-so-peercred'; connection?: AbortSignal; isConnectionActive?: () => boolean }>;
...
interface PeerModule { isConnectionActive(fd: number, pid: number, uid: number, gid: number): boolean; createListener(endpoint: string, backlog: number, onConnection: (handoff: PeerHandoff) => void, onLifecycle: (event: PeerClosure) => void, retryDelayMs: number, retryLimit: number): PeerAcceptor }
...
export function listenWithPeerIdentity(endpoint: string, maxConnections: number,
  ...
  const listener = native.createListener(endpoint, maxConnections, handoff => {
    ...
    try {
      const identity = Object.defineProperties({ pid, uid, gid, assurance: 'linux-so-peercred' as const }, {
        connection: { value: connection.signal },
        isConnectionActive: { value: () => !socket.destroyed && !connection.signal.aborted && native.isConnectionActive(fd, pid, uid, gid) },
      });
