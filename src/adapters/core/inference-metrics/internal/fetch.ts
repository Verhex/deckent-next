...
export function readInferenceMetrics(rawInput: InferenceMetricsReadInput, resolveHost: InferenceMetricsLookup = systemLookup): Promise<InferenceMetricsBody> {
  ...
  return checkedAddresses(url.hostname, resolveHost).then(addresses => new Promise<InferenceMetricsBody>((resolve, reject) => {
    let settled = false;
    let req: ClientRequest | undefined;
    const finish = (fn: () => void) => { if (!settled) { settled = true; clearTimeout(deadline); fn(); } };
    // The request `timeout` only bounds socket idleness; this deadline bounds the whole read across every address.
    ...
    const attempt = (index: number) => {
      ...
      current.on('error', () => {
        if (!settled && !responded && index + 1 < addresses.length) { attempt(index + 1); return; }
