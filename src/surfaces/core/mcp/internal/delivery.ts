...
import { ModelInvocationStoreError, type ModelInvocationDelivery } from '#engine/index.js';
...
export function modelToolDelivery(id: RequestId, maximum: number): ModelInvocationDelivery {
  ...
  if (available <= 0n) throw new ModelInvocationStoreError('MODEL_INVOCATION_RESULT_LIMIT');
