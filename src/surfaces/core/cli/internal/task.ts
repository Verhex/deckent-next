...
import type { WorkspaceIntegrationInspection, IntegrationQuery, WorkspaceIntegrationApplication, IntegrationCommand, WorkspacePatch } from '#engine/index.js';
...
export async function taskCommand(argv: readonly string[], context: CommandContext): Promise<void> {
  ...
  if (!['execute', 'evaluate', 'patch-prepare', 'patch-preview', 'integration-check', 'integration-prepare', 'integration-inspect'].includes(action ?? '')) throw usage();
  const allowed = action === 'evaluate' ? [...identityFlags, '--command-id', '--expected-revision'] : action === 'integration-prepare' ? [...identityFlags, '--command-id', '--proposal'] : action === 'integration-inspect' ? [...identityFlags, '--command-id'] : identityFlags;
