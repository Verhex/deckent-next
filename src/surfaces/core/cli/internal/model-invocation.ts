...
import { parseModelInvocationCommand, parseModelInvocationPurgeCommand, parseModelInvocationQuery, type ModelInvocationCommand,
...
import type { ModelInvocationPurgeResult, ModelInvocationResult, ModelInvocationInspection } from '#engine/index.js';
...
interface Parsed { action: 'invoke' | 'invocation' | 'purge-content'; source?: string; language?: string; json: boolean; help: boolean }
function parse(argv: readonly string[]): Parsed {
  if (argv[0] !== 'models' || (argv[1] !== 'invoke' && argv[1] !== 'invocation' && argv[1] !== 'purge-content')) throw ErrorRegistry.createError('CLI_USAGE');
...
export async function modelInvocationCommand(argv: readonly string[], context: CommandContext): Promise<void> {
  ...
  if (args.help) { emit(args.action === 'purge-content' ? t('cli.help.modelsPurgeContent', {}, locale) : t('cli.help.modelsInvocation', {}, locale), sinks); return; }
