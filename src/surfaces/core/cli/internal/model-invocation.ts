...
interface Parsed { action: 'invoke' | 'invocation'; source?: string; language?: string; json: boolean; help: boolean }
function parse(argv: readonly string[]): Parsed {
  if (argv[0] !== 'models' || (argv[1] !== 'invoke' && argv[1] !== 'invocation')) throw ErrorRegistry.createError('CLI_USAGE');
...
export async function modelInvocationCommand(argv: readonly string[], context: CommandContext): Promise<void> {
  ...
  if (args.help) { emit(t('cli.help.modelsInvocation', {}, locale), sinks); return; }
  ...
  if (args.action === 'invoke') {
    ...
    emit(result, { ...sinks, json: args.json, render: item => render(item.receipt, locale, item.replayed) });
  } else {
    ...
    emit(result, { ...sinks, json: args.json, render: item => render(item.invocation, locale) });
  }
}
